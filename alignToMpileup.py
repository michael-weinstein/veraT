...
class JobList():
    def __init__(self):
        ...
        self.alignment = programRunners.BWAlign(args.sampleName, args.referenceGenome, args.pe1, args.pe2, clobber = args.clobber)
        ...
        self.mPileup = programRunners.MPileup(args.sampleName, self.deduplicate.bamOut, args.referenceGenome, clobber = self.makeBAM.clobber)
        self.pileupToVCF = programRunners.PileupToVCF(args.sampleName, self.mPileup.mPileupOut, clobber = self.mPileup.clobber)
        self.commandList = [self.alignment.pe1Command,
                            self.alignment.pe2Command,
                            self.alignment.samCommand,
...
def submitJobs(jobList):
    ...
    alignPe1 = genericRunners.HoffmanJob([], 1, args.sampleName + "pe1align", tempDir, args.emailAddress, "ba", cores = jobList.alignment.threads, mock = args.mock)
    alignPe2 = genericRunners.HoffmanJob([], 2, args.sampleName + "pe2align", tempDir, args.emailAddress, args.emailConditions, cores = jobList.alignment.threads, mock = args.mock)
    makeSAM = genericRunners.HoffmanJob([alignPe1.jobID, alignPe2.jobID], 3, args.sampleName + "sam", tempDir, args.emailAddress, args.emailConditions, mock = args.mock)
    makeBAM = genericRunners.HoffmanJob([makeSAM.jobID], 4, args.sampleName + "bam", tempDir, args.emailAddress, args.emailConditions, mock = args.mock)
    deduplicate = genericRunners.HoffmanJob([makeBAM.jobID], 5, args.sampleName + "dup", tempDir, args.emailAddress, args.emailConditions, mock = args.mock)
    runMPileup = genericRunners.HoffmanJob([deduplicate.jobID], 6, args.sampleName + "pileup", tempDir, args.emailAddress, args.emailConditions, mock = args.mock)
    runPileupToVCF = genericRunners.HoffmanJob([runMPileup.jobID], 7, args.sampleName + "vcf", tempDir, args.emailAddress, "ea", memory = 10, mock = args.mock)
