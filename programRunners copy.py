...
class AddReadGroups(object):
    ...
    def __init__(self, sampleName, bamFile, sort_order = "coordinate", rgid = "1", rglb = "defaultLibrary", rgpl = "Illumina", rgpu = "defaultBarcode", validation_stringency = "LENIENT", create_index = True, clobber = False, outputDirectory = ""):
        self.sampleName = sampleName
        if not outputDirectory:
            self.outputDirectory = ""
        else:
        ...
        self.clobber = clobber
        #SANITY TEST ALL THE THINGS
        checkForRequiredFile(self.bamFile, "SAM file to convert to BAM")
        if not type(self.create_index) == bool:
            raise RuntimeError("Create index value should be passed as a boolean. Passed value: %s" %(self.create_index))
        #DONE SANITY CHECKING. FOR NOW.
        # if self.create_index:
